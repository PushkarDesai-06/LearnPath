...
export default function DashboardPage() {
  ...
  // therefore invalidates it for free — no stale path lingers on screen while
  // the new one loads, and there's no separate `loading` flag to keep in sync.
  // `data: null` marks a settled failure, so a failed fetch stops the skeleton.
  const [fetched, setFetched] = useState<{
    topicId: string;
    data: ProgressData | null;
  } | null>(null);
  const key = topicId ?? "";
  const current = fetched?.topicId === key ? fetched : null;

  useEffect(() => {
    let active = true;
    const q = key ? `?curriculumId=${key}` : "";
    api<ProgressData>(`/api/progress${q}`)
      .then((res) => active && setFetched({ topicId: key, data: res }))
      .catch((err) => {
        if (!active) return;
        ...
        setFetched({ topicId: key, data: null });
      });
    return () => {
      active = false;
    };
  ...
  // Nothing for this topic yet — first load or a switch still in flight.
  if (!current) return <DashboardSkeleton />;
